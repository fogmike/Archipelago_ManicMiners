...
*(If you aren't sure where to create or find the folder, try loading a custom level from the in-game level editor. This will create the folders if they doesn't already exist, as well as opening a file picker with its location already set to the Levels folder. You'll then select one folder above that for the Archipelago.)*
